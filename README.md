...
视频或转写音频。当前可下载的稳定版为 `v0.9.0`；`v0.9.3` 发布后请改用新版安装器。
...
`0.8.1` 及更早版本仍需手动安装一次 `0.9.0`。从 `0.9.0` 开始，游戏加载约 8 秒后会自动检查稳定版；
...
官方历史恢复依赖仍可访问的官方接口；`0.9.0` 源码已通过自动测试、脱敏日志检测，以及视频归档和本地
...
例如版本为 `0.9.0` 时使用 `v0.9.0`。先把同一提交和标签推到 Gitee，可以让 Gitee Release 的源码标签
