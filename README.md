...
当前源码版本为 `0.11.1`，状态为候选（CANDIDATE），截至 2026-09-03 等待真实客户端验收。
...
- [v0.11.1 候选说明](.github/release-notes/v0.11.1.md)

## 0.11.1 候选新增

0.11.1 源码候选包含手动文字导入：在信箱导入入口录入去信和/或来信正文及日期时间，先保存到
SQLite 持久待导入队列，再进行查看、编辑、删除、选择和批量提交。它支持仅去信、仅回信、日期或分钟
精度，以及明确保留未知来信时间；重复提交幂等，不占每日写信额度，也不创建大模型任务。待导入队列
已从主导入窗口中拆出，宽屏时作为独立窗口显示在主窗口右侧，窄屏时自动排列到下方。

既有 JSON、分享链接和官方历史导入保持原有行为。图片导入目前仍只有占位，OCR 未实现。本节只说明
源代码候选，不代表 0.11.1 已构建安装包、安装到游戏、发布或通过真实玩家端验收。修改服务代码后必须
重启 Node 本地服务；涉及前端补丁的验收还必须重启游戏。

`0.11.0` 曾于 2026-09-03 生成固定自签名的本地候选安装包，但未安装、未推送、未打标签、未发布，
并已被本次 UI 调整取代。该同名安装包不得作为 0.11.1 的构建或验收证据。
...
以下步骤适用于最近的正式版 `0.10.9`，不适用于尚未发布的 0.11.1 源码候选。
...
- 0.11.1 源码候选支持手动录入去信/来信，并通过右侧独立的持久队列窗口查看、编辑、删除、选择和批量提交。
...
- 0.11.1 手动文字导入和独立队列侧窗尚未完成真实 `.627` 客户端的完整 UI、窄屏布局和重启持久性验收。
- 本项目不生成视频，也不转写音频。
...
源码和二进制内容。0.11.1 当前只是源码候选，不得在验收和明确授权前打标签或发布。
