...
- 无历史或历史很少时临时加入独立重写的首封文风参考；随着有效通信积累自动缩短并完全退出，
  不伪造共同记忆，也不额外调用模型。
...
- 从本机已下载的定制演奏视频建立“我的上传”索引，并在该曲库提供“管理本地演奏”，支持重新扫描、曲名编辑、视频预览和时段校正。
- 创建、重命名和删除自定义歌单，保存曲目引用与排列顺序，方便编排播放队列。
- 在曲库和歌单中逐首选择、取消或全选曲目，批量加入播放队列或歌单；操作结果通过自动消失的顶部提示显示。
- 在音乐桌面确认后清空当前播放队列，不删除歌曲文件或自定义歌单。
- 在“设置”→“增强功能”中独立开关自定义歌单、批量操作和音乐桌面清空；关闭入口不会删除已有歌单。
...
- 自定义歌单只保存歌曲引用和排列顺序，不是独立曲库；原歌曲文件缺失时，歌单不会自动补齐或重新下载。
- 原生 Gizmo/桌面气泡组件尚未修复，相关入口暂时隐藏或标注为未修复。
