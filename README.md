...
当前源码版本为 `0.11.2`，状态为候选（CANDIDATE）。截至 2026-09-04，本地候选安装包已经构建，
但尚未安装、推送、打标签、发布或通过真实客户端验收；最近的可下载正式版仍是 2026-09-02 发布的 `0.10.9`。
...
固定自签名的本地候选安装包，但这只证明构建与静态校验完成，不代表已经安装、发布或通过真实玩家端验收。
修改服务代码后必须重启 Node 本地服务；涉及前端补丁的验收还必须重启游戏。
...
源码和二进制内容。0.11.2 当前只是未验收的本地构建候选，不得在验收和明确授权前打标签或发布。
