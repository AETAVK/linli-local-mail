...
在游戏信箱中写信、寄出，等待 AI 回信，再像原来一样打开阅读。来往信件保存在电脑上，
后续回信会参考已有通信，让交流有所延续。

没有旧信也可以开始：补丁内置了林离的角色设定和初期文风参考。这是社区重建的回信体验，
不是官方写信服务；实际语气和内容会受到所选 AI 模型的影响。生成回信需要连接你配置的模型服务。

### 把旧信整理回信箱

点击信箱中“我的信箱”右侧的“导入”，可按手头的备份选择方式：

- **文字录入**：填写去信、来信和时间，也可以只录入其中一封。录入后先放入右侧待导入队列，
  检查或修改文字与时间，再勾选提交；支持全选和批量导入。
- **JSON 备份**：一次导入多个信件备份文件。
- **分享链接**：粘贴官方信件分享链接，一行一个。只有链接仍能访问时才可使用。

还可以在确认后尝试找回当前账号的官方历史信件，前提是电脑上保留了可用登录信息，且官方接口仍可访问。
已有的视频回信可保存到本机并在游戏中播放；补丁不会生成新的回信视频。图片识别导入暂未开放。

### 选择自己想用的 AI

在“设置”→“本地回信”→“模型管理”中添加模型服务商和模型，填写服务地址与 API Key，
再选择当前使用的模型。可以保存多个配置，随时切换，并按模型支持情况调整思考档位。

模型账号和调用费用由玩家自行安排。API Key 会加密保存在本机，不会随信件一起导出。

### 整理音乐，安排播放顺序

- **自定义歌单**：把喜欢的曲目加入歌单，调整顺序，也可以重命名或删除歌单。歌单用于组织已有曲目，
  不会复制音乐文件；删除歌单不会删除原歌曲。
- **批量操作**：在曲库或歌单中逐首勾选，也可以全选，一次把多首歌曲加入播放队列或歌单。
- **音乐桌面清空**：点击“清空”并确认，即可移除当前播放队列中的曲目，不会删除音乐文件或自定义歌单。
- **我的上传**：找回电脑里已经下载的定制演奏视频。在该曲库打开“管理本地演奏”，可重新扫描、
  修改曲名、预览视频和调整播放时段；无法恢复本机没有的文件，也不提供新的官方上传或演奏生成服务。

在“设置”→“增强功能”中，可以分别开关自定义歌单、批量操作和音乐桌面清空。
三个开关互不影响，关闭歌单入口也会保留已有歌单。

### 更方便地使用游戏

通过左上角的“曲库 / 信箱”页签切换页面，隐藏离线版的 UID 水印。
在“设置”→“补丁版本”中查看版本并检查更新，也可以使用自动更新检查；安装升级仍需先退出游戏。

安装包已包含所需运行组件，普通玩家无需另装 Node.js 或手动启动命令行，按上方安装步骤操作即可。
